...
- **代码 review 重审确认与优化（2026-08-27，B1-B6）**：`src/latexSuggest.ts`。
  ...
  - **B7（已修复，用户报告 `\sec`→`\\section`）**：Monaco `getWordUntilPosition` 的单词不含 `\`（`\sec`→word=`sec`，startColumn=2）。切换前若是 `\`，补全 `range` 只覆盖 `sec`，插入片段 `\section` 时残留前导 `\` → `\\section`。修复：`provideCompletionItems` 里若词首前一字符为 `\`（charCode 92）则 `startColumn -= 1`，使替换区间含 `\`，插入即 `\section{}`（单反斜杠）。验证：模型 `\sec` 处 word=`sec`、startColumn=2、前导 `\`（92），修复后 startColumn→1。**注**：真机键盘复测受 Windows IME 拦截（`\`→`、`，troubleshooting 已记录），故以确定性 API 测 + 逻辑核验替代；B1 已确认引擎保留 `\` 字面，故区间含 `\` 后结果为单反斜杠。
