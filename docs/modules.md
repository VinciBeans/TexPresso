...
- **已知问题与待办（2026-08-25 记录，未实现）**：① **SyncTeX 正向定位到「未加载页」无法一次跳转到位**——预览采用分页 DOM 虚拟化（只挂载视口窗口内页），当正向定位（Ctrl+点击源码 → PDF）目标页**不在挂载窗口**时，「展开窗口挂载目标页 + 平滑滚动居中」在页高未落定/动画未到位时**一次跳不到位**。修复方向：跳转前确保目标页高度已预热（`pageH1`）+ 改为瞬间定位（去 `smooth`）或等布局稳定后再滚动。② **页码跳转功能**——预览工具条提供「输入页码 → 跳转到指定 PDF 页」的输入框（当前页码指示器 `1 / 3` 仅展示、不可输入）。
