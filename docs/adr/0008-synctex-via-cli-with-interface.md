...
- **Windows 实测定稿（2026-08-25，本机 TeX Live 2026 / synctex 1.5）**：
  ...
  - `edit -o "page:x:y:pdf" -d <tmp>`：输出 `Input:<源文件>`（**正斜杠 + `./`**，如 `E:/Works/tex-presso/000test/./main.tex`）、`Line:<n>`、`Column:-1`、`Offset:0`、`Context:`。命中点可能指向生成文件（`.toc`/`.aux`，如点击目录区 → `main.toc`）。`Input` 路径需归一化（剥 `./`、统一正斜杠）才能与已打开的标签匹配——见 `project.resolvePath` 修复。
